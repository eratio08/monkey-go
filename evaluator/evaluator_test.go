...
func TestEvalIntegerExpression(t *testing.T) {
	...
	for _, tt := range tests {
		...
		testIntegerObject(t, evaluated, tt.expedted)
	}
}
...
func TestErrorHandling(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if errorObj.Message != tt.expected {
			t.Errorf("Wrong error message, expected=%q, got=%q", tt.expected, errorObj.Message)
		}
	}
}
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong parameters, got %+v", len(fn.Parameters))
	}
...
func TestStringConcatenantion(t *testing.T) {
	input := `"Hello" + " " + "World!"`
...
func TestArrayLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not an Array, got %T", evaluated)
	}
...
func TestHashLiterals(t *testing.T) {
  ...
  false: 6
	...
	for expectedKey, expectedValue := range expected {
		...
		if !ok {
			t.Errorf("no pair for key %p in Pairs", &expectedKey)
		}
